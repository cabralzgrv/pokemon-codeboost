...
function createCardPokemon(code, type, nome, imagePok) {
  ...
  codePokemon.textContent = (code < 10) ? `#00${code}` : (code < 100) ? `#0${code}` :
  (code >= 100) ? `#${code}` : '';
...
function openDetailsPokemon() {
  ...
  .then(response => {
    ...
    function listingWeaknesses() {
      ...
      .then(response => {
          ...
          weaknessess.forEach(item => {
            ...
            areaWeak.appendChild(itemListWeak); // add to ul
    ...
    mainAbilitiesPokemonModal.textContent = primeiraLetraMaiuscula(infoPokemon.mainAbilities);
...
function showMorePokemons() {
  listingPokemon(`https://pokeapi.co/api/v2/pokemon?limit=9&offset=${countPagPoke}`);

  countPagPoke = countPagPoke + 9;
}
...
.catch((error) => {
  if(error.response) {
    ...
    alert ('Não foi encontrado nenhum resultado nessa pesquisa');
  }
})
